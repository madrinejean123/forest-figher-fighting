...
class Fire(Supervisor):
    ...
    def ignite(self, tree):
        ...
        root = self.root.getField('children')

        node = '''Shape {
  appearance Appearance {
    material Material { emissiveColor 1 0.3 0 }
  }
  geometry Sphere { radius 0.4 }
    ...
    def burn(self, tree):
        print("UPDATE_FIRE =", self.update_fire)
        if self.update_fire:
            tree.fire_count += 1
            if tree.fire_count % 10 == 0:
                tree.fire_scale *= 1.05
    ...
    def run(self):
        ...
        while True:
            ...
            if not start_fire_now:
                for robot in self.robots:
                    if robot.name == "Mavic 2 PRO":
                        try:
                            print("ROBOT:", robot.name, "ALT:", robot.altitude())
                            if robot.altitude() > 40:
                                print("FIRE STARTED AT ALTITUDE:", robot.altitude())
                                start_fire_now = True
                        except:
                            pass

            if start_fire_now:
                print("ENTERED FIRE BLOCK"); print("FIRE_CLOCK =", self.fire_clock)
                self.fire_clock += 1
                if self.fire_clock >= self.FIRE_DURATION:
                    print('🔥 FIRE STARTED (VISIBLE WINDOW)')
                    self.update_fire = True
                    self.fire_clock = 0
                else:
                    self.update_fire = False
                    if self.fire_clock < 10:
                        self.update_fire = False
                if self.fire_clock >= self.FIRE_DURATION:
                    print('🔥 FIRE ACTIVE WINDOW')
                    self.update_fire = True
                    self.fire_clock = 0
                else:
                    self.update_fire = False
                ...
                if self.fire_clock >= self.FIRE_DURATION:
                    print("🔥 UPDATE_FIRE TURNED TRUE")
                    self.update_fire = True
                    self.fire_clock = 0
                else:
                    self.update_fire = False

                print("DEBUG -> clock:", self.fire_clock, "update:", self.update_fire)
                extinction = []
